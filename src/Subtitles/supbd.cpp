...
SupBD::SupBD(
  QString fileName, SubtitleProcessor *subtitleProcessor)
...
QImage SupBD::image() { return _bitmap.image(_palette); }
...
int SupBD::numFrames() { return subPictures.size(); }
...
qint64 SupBD::endTime(
  int index)
{
  return subPictures[index].endTime();
}

qint64 SupBD::startTime(
  int index)
{
  return subPictures[index].startTime();
}

qint64 SupBD::startOffset(
  int index)
{
...
void SupBD::readAllSupFrames()
{
  ...
  try {
    while (index < bufsize) {
      ...
      switch (segment.type) {
        ...
        {
          if (pcs.pts != -1) {
            out = QString("PDS ofs:0x%1, size:0x%2")
        ...
        {
          if (pcs.pts != -1) {
            out = QString("ODS ofs:0x%1, size:0x%2")
                    .arg(QString::number(index, 16), 8, QChar('0'))
                    .arg(QString::number(segment.size, 16), 4, QChar('0'));
...
#endif
                subtitleProcessor->print(
                  QString("%1, img size: %2*%3\n").arg(out).arg(QString::number(ods.width)).arg(QString::number(ods.height)));
              }
              else {
        ...
        case 0x16:
        {
          if (pcs.pts != -1) {
            if (pic.numCompObjects() != 0) {
              subtitleProcessor->printX(QString("#> %1 (%2)\n").arg(QString::number(++subCount)).arg(TimeUtil::ptsToTimeStr(pcs.pts)));
            }
          ...
          for (int i = 0; i < pcs.forcedFlags.keys().size(); ++i) {
            ...
            out += QString(", Object Id: %1, forced: %2").arg(QString::number(pcs.forcedFlags.keys()[i])).arg(forced ? "true" : "false");
          }
        ...
        case 0x17:
        {
          out = QString("WDS ofs:0x%1, size:0x%2")
                  .arg(QString::number(index, 16), 8, QChar('0'))
                  .arg(QString::number(segment.size, 16), 4, QChar('0'));
        ...
        case 0x80:
        {
          ...
          if (pcs.pts != -1) {
            ...
            if (pic.numCompObjects() != 0 && !subPictures.isEmpty() && (pic.compNum() != subPictures.back().compNum())
                && !imagesAreMergeable(pic, subPictures.back()))
            {
              subtitleProcessor->printX(QString("#< %1 (%2)\n").arg(QString::number(++subCount)).arg(TimeUtil::ptsToTimeStr(pcs.pts)));
            }
  ...
  if (pic.startTime() != -1) {
    subPictures.push_back(pic);
    subtitleProcessor->printX(
      QString("#> %1 (%2)\n").arg(QString::number(subPictures.size())).arg(TimeUtil::ptsToTimeStr(pic.startTime())));
...
bool SupBD::imagesAreMergeable(
  SubPictureBD &currentSub, SubPictureBD &prevSub)
{
...
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
QVector<uchar> SupBD::encodeImage(
  Bitmap &bm)
#else
QList<uchar> SupBD::encodeImage(
  Bitmap &bm)
#endif
{
  uchar color;
...
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
QVector<uchar> SupBD::createSupFrame(
  SubPicture *subPicture, Bitmap &bm, Palette &pal, bool forcedOnly)
#else
QList<uchar> SupBD::createSupFrame(
  SubPicture *subPicture, Bitmap &bm, Palette &pal, bool forcedOnly)
#endif
{
  ...
  if (pal.size() > 255 && pal.alpha(255) > 0) {
    ...
    subtitleProcessor->print(
      QString("Palette had to be reduced from %1 to %2 entries.\n").arg(QString::number(pal.size())).arg(QString::number(size)));
  ...
  NumberUtil::setWord(packetHeader, 11,
                      headerPCSStart.size() +  // size
...
SupBD::SupSegment SupBD::readSegment(
  int offset)
{
  SupSegment segment;
...
PCS SupBD::parsePCS(
  SupSegment *segment, QString &msg)
{
...
ODS SupBD::parseODS(
  SupSegment *segment, QString &msg, bool forceFirst, bool &isFirst)
{
  int index = segment->offset;
  ...
  if (isFirst) {
    ...
    msg = QString("ID: %1, update: %2, seq: %3%4%5")
            .arg(QString::number(objID))
...
CompositionState SupBD::getCompositionState(
...
void SupBD::findImageArea(
  SubPictureBD *subPicture)
{
...
Bitmap SupBD::decodeImage(
  SubPictureBD *subPicture, int transparentIndex)
{
  int numImgObj = 0;
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
  ...
  if (bitmaps.size() == 2) {
    ...
    int right = (imgObj1XOfs + imgObj1Width) > (imgObj2XOfs + imgObj2Width) ? (imgObj1XOfs + imgObj1Width) : (imgObj2XOfs + imgObj2Width);
